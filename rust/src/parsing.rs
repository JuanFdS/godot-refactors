...
impl GDScriptParser {
    pub fn parse_to_ast<'a, T>(
        input: &'a str,
        rule_type: Rule,
        rule_mapper: fn(Pair<'a, Rule>) -> T,
    ) -> T {
    ...
    pub fn try_parse_to_ast<'a, T>(
        ...
        GDScriptParser::parse(rule_type, input).map(|mut pairs| rule_mapper(pairs.next().unwrap()))
    }
    ...
    pub fn to_program(parse_result: Pair<Rule>) -> Program {
        match parse_result.as_rule() {
            Rule::program => {
                let super_class: Option<String> = parse_result
                    .clone()
                    .into_inner()
                ...
                let is_tool: bool = parse_result
                    .clone()
                    .into_inner()
                    .any(Self::is_rule(Rule::tool_annotation));
    ...
    fn to_declaration<'a>(parse_result: Pair<'a, Rule>) -> Declaration {
        match parse_result.as_rule() {
            ...
            Rule::var_declaration => {
                ...
                let annotation = inner_rules
                    .clone()
                    .find(|p| p.as_rule() == Rule::annotation);
                let identifier = inner_rules
            ...
            Rule::unknown => DeclarationKind::Unknown(parse_result.as_span().as_str().into())
                .to_declaration(Some(parse_result)),
    ...
    fn to_statement(parse_result: Pair<Rule>) -> Option<Statement> {
        match parse_result.as_rule() {
            ...
            Rule::return_statement => {
                let maybe_expression = parse_result
                    .clone()
                    .into_inner()
                    .next()
            ...
            Rule::unknown => Some(Statement::new(
                Some(parse_result.clone()),
                StatementKind::Unknown(parse_result.as_span().as_str().to_owned()),
            )),
            Rule::pass => Some(Statement::new(Some(parse_result), StatementKind::Pass)),
            Rule::expression => {
    ...
    fn to_expression(parse_result: Pair<Rule>) -> Expression {
        let string_to_literal_int =
            |str: &str| ExpressionKind::LiteralInt(str.parse::<usize>().unwrap());
        ...
        match &parse_result.as_rule() {
            ...
            &Rule::message_send => {
                ...
                Expression::new(
                    pair,
                    ExpressionKind::MessageSend(Box::new(receiver), message_name.into(), vec![]),
                )
            }
            &Rule::first_operator => Expression::new(
                pair,
                Self::to_expression(parse_result.into_inner().next().unwrap()).kind,
            ),
            &Rule::variable_usage => Expression::new(
                pair,
                ExpressionKind::VariableUsage(parse_result.as_span().as_str().into()),
            ),
            &Rule::literal_int => {
            ...
            &Rule::binary_operation => {
                ...
                Expression::new(
                    pair,
                    ExpressionKind::BinaryOperation(Box::new(lhs), operator.into(), Box::new(rhs)),
                )
            }
            &Rule::unknown => Expression::new(
                pair,
                ExpressionKind::Unknown(parse_result.as_span().as_str().to_string()),
            ),
            &Rule::expression => Self::to_expression(parse_result.into_inner().next().unwrap()),
            _ => panic!(),
        }
    }
}
